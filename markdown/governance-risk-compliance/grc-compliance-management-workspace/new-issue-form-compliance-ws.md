...
**Note:** Use the bulb icon to get suggestions on who must the issue be assigned to. The bulb icon only appears if you have the GRC: Predictive Intelligence application activated, the form is saved, the **Assigned to** field is not disabled, and the GRC Property is selected as Similarity Analysis. For more information, see [Governance, Risk, and Compliance properties](../grc-common-functions/grc-properties.md).

Starting with Version 12.0.1, the assigned-to user receives an email notification when the issue manager [requests more information](../policy-and-compliance-management/remediate-issue.md).
...
-   The issue manager receives an email notification when the assigned-to user [requests more information](../policy-and-compliance-management/remediate-issue.md).
