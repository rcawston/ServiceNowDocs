...
Service order that is still in the capture stage or the manual entry of the service order is not complete. The data capture phase is when your instance is capturing full service order details from the TMF641. **Note:** The Service Order Open API is a ServiceNow implementation of the TMForum TMF641 Service Ordering Open API specification. To learn more, see [Service Order Open API](../api-reference/rest-apis/service-order-open-api.md).
