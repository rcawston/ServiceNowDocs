...
Response drafted by the agent to resolve the case. To copy content from a response template and add to this field, select the response template icon \(![response template icon](../image/response-template-icon.png)\). For more information on how to use a response template, see [Use response templates to add information to cases](../../customer-service-management/csm-aw-agent-use-response-template.md).
