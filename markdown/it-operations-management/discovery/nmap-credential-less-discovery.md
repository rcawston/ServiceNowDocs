...
A MID Server administrator can [install Nmap](../../servicenow-platform/mid-server/install-nmap-on-mid-server.md) on individual MID Servers running on a Windows host. Those MID Servers can then discover some basic information about CIs in your network when normal authentication fails.

**Important:** Self-hosted customers whose network security does not permit downloads from `install.service-now.com` must install and configure Nmap manually on their system. Refer to [Install Nmap on a self-hosted system](../../servicenow-platform/mid-server/install-nmap-on-mid-server.md) for instructions.
