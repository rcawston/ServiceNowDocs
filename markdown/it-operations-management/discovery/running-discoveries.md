...
-   **Credentials**: Configure the MID Servers with the login credentials they need to query the devices in the network. The MID Server tries all available credentials on each discovered device, then creates an affinity for any successful credentials. For more information, see [Credential affinity for Discovery and Orchestration](../../platform-security/connections-and-credentials/c_CredentialAffinity.md).
