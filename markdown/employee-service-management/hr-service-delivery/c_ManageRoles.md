...
**Note:** IT System Administrators \(admin\) can still impersonate ServiceNow users. When impersonating a user with an HR scope-protected role, an admin cannot access features granted by that role unless the admin already possesses those HR scope-protected roles. For more information on impersonating a user, see [Impersonate a user](../../platform-administration/user-administration/c_ImpersonateAUser.md).
...
-   Remove the HR Administrator \[sn\_hr\_core.admin\] role from System Administrator \[admin\].
    ...
    -   Change the scope of the application to Human Resources: Core application. For information on changing the scope, see [Contextual development edit messages](../../application-development/c_WarningMessages.md).
    -   Reveal the **Run as** field. For information on revealing hidden fields on a form, see [Configuring the form layout](../../platform-administration/configure-form-layout.md).
