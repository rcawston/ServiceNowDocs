...
-   Set the application scope to the application associated with the messaging channel using the application picker. For more information, see [Application picker](../../application-development/c_ApplicationPicker.md).
...
Order number for each channel in which they appear for agent on the Compose Message form when composing messages. The channel that has the lowest order number is displayed first.For information on the Compose Message form available in CSM Configurable Workspace, see [Initiate messaging conversations from CSM Configurable Workspace](../../customer-service-management/agent-init-messg-csm-cws.md).
