...
7.  Select the **Search Mapping** tab to change the search application for the chat experience.
    ...
    **Note:** Although you can change the **Search UI \(EVAM\) Configuration**, the Virtual Agent Search \(EVAM\) configuration provides the appropriate card views and lists of Virtual Agent search results. If you want to create your own EVAM configuration for use, see [Create an EVAM definition](../../servicenow-platform/entity-view-action-mapper-evam/define-composite-dataset.md).
