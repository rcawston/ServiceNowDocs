...
List of available languages in Virtual Agent.
 ...
 Only installed languages are displayed \(the language plugins are activated\). If a language appears as unavailable, it means the language plugin is installed, but it has not been activated in Virtual Agent settings or in the Localization Framework. For more information, see [Configure Natural Language Understanding in Virtual Agent](configure-nlu-settings.md) and [Languages table](../../platform-administration/system-localization/r_LanguagesTable.md).
