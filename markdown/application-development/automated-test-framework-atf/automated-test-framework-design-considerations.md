...
**Note:** If you have two or more resource conflicting tests, see [Mark tests as mutually exclusive](mutual-exclusion-rule.md#mark-tests-as-mutually-exclusive) to create a mutual exclusion rule that prevents them from running in parallel.

## Parameterized testing

Run a test multiple times with different test data for each run. Create parameters to store test data for each test run. See [Parameterized test components](parameterized-tests.md#parameterized-test-components) for more information.
