---
title: What ServiceNow product solves your problem
description: Use this table to find ServiceNow products that help solve your problem.
locale: en-US
release: australia
topic_type: concept
last_updated: "2026-03-12"
reading_time_minutes: 5
breadcrumb: [Find ServiceNow developer products quickly, Learning about developing on the ServiceNow AI Platform, Building applications]
---

# What ServiceNow product solves your problem

Use this table to find ServiceNow products that help solve your problem.

<table id="table_s2j_c4b_rvb"><thead><tr><th>

I want to

</th><th>

Using

</th><th>

ServiceNow product or topic

</th></tr></thead><tbody><tr><td>

Get an overview of developing

</td><td>

 

</td><td>

-   [What is ServiceNow](http://www.servicenow.com)?
-   [Get a development instance](get-dev-instance.md)
-   [Demos](https://www.servicenow.com/lpdem/demonow-all.html?campid=99434&cid=p:all:dg:allterms:prsp:allmatch:GOOG_ALBU_PRSP_DemoNowALLDemoPages:ams:all&s_kwcid=AL!11692!3!579286898115!e!!g!!what%20is%20servicenow&ds_c=GOOG_AMS_All_EN_DEMANDGEN_ALBU_PRSP_Brand_EXA_Top&cmcid=71700000065323099&ds_ag=ServiceNow+What+Is_EXA&cmpid=58700005783665150&ds_kids=p63311635460&gclid=Cj0KCQiAn4SeBhCwARIsANeF9DLHaN8E-SrxM9UL14y_GRds_mudiBr6G2sCXdSZUfVKJhWaKYTkuacaAt9JEALw_wcB&gclsrc=aw.ds)
-   [How the ServiceNow AI Platform works](../platform-administration/how-now-platform-works.md)
-   [Licensing](licensing.md)
-   [What is low-code development?](https://www.servicenow.com/workflows/creator-workflows/what-is-low-code.html)

</td></tr><tr><td>

Get help developing applications

</td><td>

 

</td><td>

-   [Getting started on the ServiceNow AI Platform](../platform-administration/get-started-now-platform.md)
-   [ServiceNow developer community](https://www.servicenow.com/community/developer/ct-p/Developer)
-   [Customer support](https://support.servicenow.com/kb?id=kb_article_view&sysparm_article=KB1000420)
-   [Knowledge base](https://support.servicenow.com/kb?id=kb_home)
-   [Customer Success Center](https://www.servicenow.com/success.html)
-   [ServiceNow training](https://developer.servicenow.com/dev.do#!/learn)
-   [Additional support](r_support-servicenow-developers.md)
-   [Known error portal](https://support.servicenow.com/kb?id=known_error_portal)

</td></tr><tr><td>

Plan to create an application

</td><td>

 

</td><td>

-   [Delegated development](delegated-development-and-deployment/c_DelegatedDevelopment.md)
-   [Delegated development in Creator Studio](creator-studio/creator-studio-admin-collab-tasks.md)
-   [Delegated development in App Engine Studio](app-engine-studio/aes-app-dev-workflow.md)
-   [Source control](servicenow-studio-classic/c_SourceControlIntegration.md)
-   [Common Service Data Model](../servicenow-platform/common-service-data-model-csdm/csdm-landing-page.md)

</td></tr><tr><td rowspan="3">

Create an application

</td><td>

-   No code
-   Next Experience

</td><td>

-   [Creator Studio](creator-studio/creator-studio-landing.md)
-   [App Engine Studio](app-engine-studio/aes-overview.md)

</td></tr><tr><td>

-   Coding and scripting
-   Core UI tech stack
-   Forms and lists

</td><td>

-   [ServiceNow Studio](servicenow-studio-classic/servicenow-studio-landing.md)
-   [ServiceNow Extensions for Visual Studio Code](vs-code.md)
-   [REST APIs](../api-reference/rest-api-explorer/c_RESTAPI.md)
-   [Guided Application Creator](guided-application-creator/guided-app-creator.md)
-   [ServiceNow AI Platform® forms, fields, and lists](../platform-administration/now-platform-forms-fields-lists.md)

</td></tr><tr><td>

Mobile platforms

</td><td>

-   [Mobile App Builder](../mobile/mab-concept.md)
-   [Mobile Card Builder](../mobile/mobile-card-builder.md)

</td></tr><tr><td rowspan="4">

Customize an application

</td><td>

UI pages

</td><td>

-   [UI Builder](ui-builder/ui-builder-overview.md)
-   [Workspace Builder](workspace-builder/configure-workspace-builder.md) \(in App Engine Studio only\)

</td></tr><tr><td>

Data, tables

</td><td>

[Table Builder](form-builder-glide-family-release/tb-landing-page.md)

</td></tr><tr><td>

Scripts

</td><td>

-   No-code scripting \([Workflow Studio - Building custom actions](../build-workflows/workflow-studio/actions.md)\)
-   [ServiceNow Studio](servicenow-studio-classic/servicenow-studio-landing.md)
-   [Code editor](../api-reference/scripts/monaco-editor.md)
-   [Client-side scripting](https://developer.servicenow.com/dev.do#!/reference/api/tokyo/client)
-   [Server-side scripting](https://developer.servicenow.com/dev.do#!/learn/courses/tokyo/app_store_learnv2_scripting_tokyo_scripting_in_servicenow/app_store_learnv2_scripting_tokyo_server_side_scripting/app_store_learnv2_scripting_tokyo_server_side_apis)
-   [Script Debugger](../api-reference/scripts/script-debugger.md)
-   [ServiceNow CLI](servicenow-cli/servicenow-cli.md)

</td></tr><tr><td>

Service Portals

</td><td>

-   [Create and edit a page using the Service Portal Designer](../platform-user-interface/service-portal/t_ConfigureAPage.md)
-   [Service Portal widgets](../platform-user-interface/service-portal/service-portal-widgets.md)
-   [Configuring Next Experience themes and preferences](../platform-user-interface/config-next-experience-themes-prefs.md)

</td></tr><tr><td>

Add automation

</td><td>

 

</td><td>

-   [Automation Discovery](../now-intelligence/automation-discovery.md)
-   [Workflow Studio](../build-workflows/workflow-studio/workflow-studio.md)
-   [Exploring Decision Tables](../build-workflows/workflow-studio/decision-designer-overview.md)
-   [Playbooks](../build-workflows/workflow-studio/process-automation-designer.md)
-   [Journey designer](../employee-service-management/employee-service-management/jny-dsgnr-landing-page.md)
-   [Classic Business rules](../api-reference/business-rules-classic/c_BusinessRules.md)

</td></tr><tr><td rowspan="2">

Integrate my applications with ServiceNow

</td><td>

Automation and flows

</td><td>

-   [Integration Hub](../integrate-applications/integration-hub/integrationhub.md)
-   [Flows in Workflow Studio](../build-workflows/workflow-studio/exploring-flows.md)

</td></tr><tr><td>

APIs

</td><td>

-   [REST API reference](../api-reference/rest-apis/api-rest.md)
-   [Server API reference](../api-reference/server-api-reference/api-server.md)
-   [Client API reference](../api-reference/api-client.md)
-   [Browse APIs by product](../api-reference/api-product-type.md)
-   [Workflow Studio - Building custom actions](../build-workflows/workflow-studio/actions.md)
-   [Integration Hub Remote Process Sync](../integrate-applications/integration-hub-remote-process-sync/integrationhub-remote-process-sync.md)

</td></tr><tr><td rowspan="2">

Secure an application

</td><td>

No code \(App Engine Studio only\)

</td><td>

[Configure AES personas and roles](app-engine-studio/aes-personas-roles.md)

</td></tr><tr><td>

Coding and scripting, Core UI tech stack

</td><td>

-   [Edge Encryption](../platform-security/edge-encryption/edge-encryption.md)
-   [Using UI policies](../platform-administration/t_CreateAUIPolicy.md)
-   [Managing roles](../platform-administration/user-administration/ua-creating-roles.md)
-   [ServiceNow® access control](../platform-security/c_SNCAccessControl.md)
-   [User administration](../platform-administration/user-administration/c_UserAdministration.md)

</td></tr><tr><td rowspan="2">

Engage customers

</td><td>

Customize chat bot dialogs

</td><td>

-   [Getting started with Virtual Agent Designer](../conversational-interfaces/virtual-agent/conversation-designer-virtual-agent.md)
-   Conversational Analytics Dashboard

</td></tr><tr><td>

Artificial intelligence

</td><td>

-   [Analyze and optimize business processes](../now-intelligence/c_performanceAnalyticsAndReporting.md) NOW Intelligence
-   [Predictive Intelligence](../intelligent-experiences/predictive-intelligence/predictive-intelligence.md)

</td></tr><tr><td rowspan="2">

Test an application

</td><td>

For App Engine Studio apps

</td><td>

[Automated Test Framework](automated-test-framework-atf/automated-test-framework.md)

</td></tr><tr><td>

Coding, Core UI tech stack

</td><td>

-   [Automated Test Framework](automated-test-framework-atf/automated-test-framework.md)
-   [Test Management applications](../it-business-management/test-management/test-management-overview.md)
-   [Script Debugger](../api-reference/scripts/script-debugger.md)
-   [Script Tracer](../api-reference/scripts/script-tracer.md)
-   [Session Log](../api-reference/scripts/script-debugger.md)
-   [Impersonate a user](../platform-administration/user-administration/t_ImpersonateAUserInUI16.md)
-   [ServiceNow CLI](servicenow-cli/servicenow-cli.md)

</td></tr><tr><td rowspan="2">

Deploy an application

</td><td>

No code \(App Engine Studio only\)

</td><td>

[App Engine Management Center](app-engine-management-center/managing-app-development-using-aemc.md)

</td></tr><tr><td>

Coding, Core UI tech stack

</td><td>

-   [ServiceNow application repository](application-repository-self-hosted/app-repo.md)
-   [Cloud Provisioning and Governance](../it-operations-management/cloud-configuration-governance/cloud-management-v2-landing-page.md)

</td></tr><tr><td>

Maintain an application

</td><td>

 

</td><td>

-   [Maintaining and monitoring the ServiceNow AI Platform](../platform-administration/maintain-monitor-now-platform.md)
-   [Maintaining and monitoring the ServiceNow AI Platform](../platform-administration/maintain-monitor-now-platform.md)[System update sets](system-update-sets/system-update-sets.md)
-   [Service Mapping](../it-operations-management/service-mapping/c_ServiceMappingOverview.md)
-   [Instance Data Replication](../servicenow-platform/instance-data-replication-idr/instance-data-replication.md)
-   [MetricBase](../servicenow-platform/metricbase/metricbase.md)

</td></tr><tr><td>

Create reports

</td><td>

 

</td><td>

-   [CMDB 360 view in CMDB Workspace](../servicenow-platform/configuration-management-database-cmdb/cmdb-workspace-cmdb360-view.md)
-   [Analyze and optimize business processes](../now-intelligence/c_performanceAnalyticsAndReporting.md) - Performance Analytic Reports
-   [Analytics Hub](../now-intelligence/performance-analytics/c_UsePerformanceAnalyticsScorecards.md)
-   [Analytics Center](../now-intelligence/analytics-center.md)
-   Conversational Analytics Dashboard

</td></tr><tr><td>

Copy an instance

</td><td>

 

</td><td>

[Instance Data Replication](../servicenow-platform/instance-data-replication-idr/instance-data-replication.md)

</td></tr></tbody>
</table>