...
1.  In Workflow Studio, test the **Use ERP Data** action by selecting the model in the **System** field of the Test Action modal and then specifying the inputs.
    -   For more information on testing actions, see [Test an action](../../build-workflows/workflow-studio/test-action.md).
    -   For more details on inputs for the action, see [Use ERP Data action details for flows](erp-canvas-flow-reference-2.md).
2.  After the test runs, view the action execution details. For more information, see [Flow execution details](../../build-workflows/workflow-studio/flow-execution-details.md).
