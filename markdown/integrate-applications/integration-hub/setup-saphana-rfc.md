...
Option to use a MID Server to run the SFTP step. This field is available when **Define Connection Inline** is selected from the Source Connection list and **Auto-Select MID Server** is selected from the MID Selection list.To learn more about how a MID Server is selected during runtime, see [MID Server selection](../../servicenow-platform/mid-server/c_MIDServerSelector.md).
