...
-   [`TMFTopicEventAPIUtilOOB`](../server-api-reference/tmftopiceventapiutiloobScopedAPI.md#): Contains functions to handle API requests that are triggered by [external trigger definition flows](../../integrate-applications/integration-hub/set-up-flow-ihub.md) that have the actions `AlarmCreateNotification`, `AlarmChangeNotification`, `AlarmDeleteNotification` which create, update, and delete events.
