...
-   The session language matches the language field.
 ...
 If no match is found, any fallback languages are checked for a match \(see [Set a fallback language](../../platform-administration/system-localization/set-fallback-language.md)\). If a match is still not found, the message value \(2nd parameter\) is returned.
