...
If true, the On-Call escalation path is followed when a user group is added to a conference call. However, the time between reminders is set to 60 secs. Users can create properties for required task types such as incident. Example: For Incident, the property key would be **com.snc.notify.incident.conference\_call\_follow\_on\_call\_escalation**.**Note:** The property is enabled only when the [Notify](../../servicenow-platform/notify/t_ActivateNotify.md) plugin \(com.snc.notify\) is active.
...
-   The property is enabled only when the [Notify](../../servicenow-platform/notify/t_ActivateNotify.md) plugin \(com.snc.notify\) is active.
