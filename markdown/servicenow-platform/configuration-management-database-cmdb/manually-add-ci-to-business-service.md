...
-   Verify that the CI type for the configuration item \(CI\) that you are planning to add, exists. If necessary, create the CI type as described in [Create CI types for Service Mapping and Discovery](../../it-operations-management/discovery-and-service-mapping-patterns/t_CreateCITypeForSM.md).
...
6.  If Service Mapping is activated, add a discoverable outgoing connection for the manually added CI:
    ...
    3.  Configure attributes for the entry point as described in [Entry points attributes](../../it-operations-management/service-mapping/r_EntryPointsforBizSvcDef.md).
