...
Use the [CMDBDuplicateTaskUtils](../../api-reference/server-api-reference/CMDBDuplicateTaskUtilsAPI.md) API to manually create a de-duplication task in which all duplicate CIs are specified. The de-duplication tasks that you create manually and the automatically created tasks, are stored in the same table \(\[reconcile\_duplicate\_task\]\) and are processed in the same manner.
