...
CSD allows an administrator to create all the records necessary to deploy software from service catalog requests, including software models and catalog items. You can use the CSD application to automate the deployment and [revocation](c_SCCMSoftwareRevocation.md#) of software from an SCCM host using the custom [SCCM activity pack](c_SCCMActivityPack.md). You can define lease periods for software distributed from the Service Catalog and allow lease extensions in some cases, pending approvals. Deployment is accomplished using Orchestration activities and workflows.
