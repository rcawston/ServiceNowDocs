...
When a user submits a time card, the [time sheet policy](time-sheet-policies.md) assigned to the user is used for validation. If there’s no time sheet policy assigned to the user, the default time sheet policy is used. Business rules help prevent a user from submitting a time card in case any violation is found.
