...
During out-of-office time, the user manager can [Delegate time sheet approvals to another user](../../it-business-management/time-card-management/delegate-time-sheet-approvals.md) with the required roles as the time sheet approver. For the specified time period, the delegated user also receives all the time sheets submitted by the time card users to process.
...
-   If a time sheet is rejected, it gets listed in the notifications for the rejected time sheets under the show notifications icon \(![Show notifications icon](../image/NotificationIcon.png)\) on [Time Sheet Portal](worker-portal.md#section_sth_2wd_vz) and is moved back to the submitter to adjust the corrections and resubmit it. For more information, see [Submit a time sheet](submit-time-sheet.md).
