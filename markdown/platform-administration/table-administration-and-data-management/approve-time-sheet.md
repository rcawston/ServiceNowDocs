...
-   If a time sheet is rejected, it gets listed in the notifications for the rejected time sheets under the show notifications icon \(![Show notifications icon](../image/NotificationIcon.png)\) on [Time Sheet Portal](worker-portal.md#time-sheet-portal) and is moved back to the submitter to adjust the corrections and resubmit it. For more information, see [Submit a time sheet](submit-time-sheet.md).
