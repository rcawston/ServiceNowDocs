...
-   Role-based field-level [access control list rules](../../platform-security/access-control/access-control-rules.md) \(ACLs\)
-   Non-scripted conditional ACLs
-   Scripted table-level ACLs
-   [Before Query business rules](../../platform-security/bp-before-query-business-rules.md)
-   [Domain separation](ai-search-domain-separation.md)
-   [User criteria](../../servicenow-platform/service-catalog/t_CreateAUserCriteriaRecord.md) for records indexed from the Knowledge \[kb\_knowledge\] and Catalog Item \[sc\_cat\_item\] tables
...
More computationally expensive alternate content security implementation that post-filters search query results. The system applies security filters to the search query just as early binding security does. After retrieving records that match the filtered query, it invokes the [GlideRecord.canRead\(\)](../../api-reference/server-api-reference/c_GlideRecordScopedAPI.md) method for each record. To appear in the search results, records must match the search query, pass all security filters, and return **true** for the API method.
