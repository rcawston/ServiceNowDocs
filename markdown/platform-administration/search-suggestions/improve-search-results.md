...
-   [Search Source Event \[sys\_search\_source\_event\]](understanding-search-suggestion-tables.md#search-event-per-source-table): Stores search strings and search results and ties them to a data source, for example, the suggestion came from the knowledge base table rather than the catalog table.
