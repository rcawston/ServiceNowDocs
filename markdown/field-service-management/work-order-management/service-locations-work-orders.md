...
The address information is entered in the text field or by dragging and dropping the marker. The fields auto-populate based on the address entered in the search field on a map.
 ...
 The Google Maps API key or client ID must be set to use the Google Maps API for Business. For more information, see [Set up Google Maps API](../../platform-user-interface/set-up-google-maps-api.md).
