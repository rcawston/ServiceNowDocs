...
This task describes how to create a new dynamic filter option and an associated [script include](../api-reference/scripts/c_ScriptIncludes.md).

Dynamic filter options "objectize" script includes/JavaScript, enabling them to be reused in multiple [condition builders](c_ConditionBuilder.md) and dynamic [Reference qualifiers](../platform-administration/c_ReferenceQualifiers.md). This type of filter enables you to modify a script once and have the changes automatically take effect everywhere the dynamic filter option is used.
...
1.  [Create a scripted filter](../api-reference/scripts/c_ScriptIncludes.md) as a client-callable script include or business rule.
